...
const addUser = (nomor, nama) => {
  let userData = [];
  try {
    userData = JSON.parse(fs.readFileSync("./function/users.json", "utf8"));
  } catch (error) {
    console.log(error);
    return;
  }
  ...
  fs.writeFileSync("./function/users.json", JSON.stringify(userData, null, 2));
};

const setRole = (targetNumber, newRole) => {
  try {
    const usersData = JSON.parse(fs.readFileSync("./function/users.json"));
    ...
    if (userIndex !== -1) {
      ...
      fs.writeFileSync("./function/users.json", JSON.stringify(usersData, null, 2));
